...
use artimonist::{BIP85, Error, Xpriv};
use std::str::FromStr;
...
fn bip85_define() -> Result<(), Error> {
    ...
    for (i, count) in MNEMONIC_COUNTS.into_iter().enumerate() {
        assert_eq!(master.bip85_mnemonic(0, count, English)?, MNEMONIC_WORDS[i]);
    }
    // wif
    {
        #[cfg(not(feature = "testnet"))]
...
fn bip85_derive() -> Result<(), Error> {
    ...
    for (i, &ws) in MNEMONICS.into_iter().enumerate() {
        ...
        assert_eq!(master.bip85_mnemonic(i as u32, count, English)?, ws);
    }
    // wif
